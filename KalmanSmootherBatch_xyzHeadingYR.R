...
#(2) Creation date: [1] "Tue May 24 22:06:34 2016" (adapted from xyz script)
#(3) Purpose:
...
#
#(4)Last executed: "Wed Jul 13 22:21:05 2016"
...
solutionFileName="DataContestCombined_xyzHYr.csv"
...
#make a real data value indicator variable that can be used later:
dataDummy<- ulat
dataDummy[utime+1] <- 1
...
y = cbind(ulat,ulon,uElv,uhead,uYaw)
...
if(any(emInc==4)){
  ...
  smoothHeading = (smoothData[which(emInc==4),]*attributes(shead)[[3]])+attributes(shead)[[2]]
...
if(any(emInc==5)){
  ...
  smoothYawrate = (smoothData[which(emInc==5),]*attributes(sYaw)[[3]])+attributes(sYaw)[[2]]
} else {
  #This was constant, so make no assumptions and replace entire ts with mean:
...
uDF <- data.frame(cbind(smoothLatitude, smoothLongitude, smoothElevation, smoothYawrate,
                        smoothGentimeSec), ID=as.character(smoothID))

#don't forget the original data for reference:
origY<-cbind(ulat,ulon,uElv,uhead,uYaw)
origY[which(dataDummy==0),] <- NA
oDF <-data.frame(origY,smoothGentimeSec, ID=as.character(smoothID))

names(oDF)<-c("latitude", "longitude", "elevation", "heading", "YawRate", "GentimeSec", "ID")

#Finally, stack solutions:
#e.g. rbind(data.frame(matrix(0,6,6)),data.frame(matrix(0,6,6)))
outputDF<- rbind(outputDF, uDF)
originalDF<-rbind(originalDF, oDF)
}
...
#
write.table(originalDF, file="DataContestCombined_xyzHYr_RAW.csv", sep=",", row.names = FALSE)
write.table(outputDF, file="DataContestCombined_xyzHYr_comibined.csv", sep=",", row.names = FALSE)
